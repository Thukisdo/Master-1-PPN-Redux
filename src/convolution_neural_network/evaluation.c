...
void update_score(Layer* output_layer, f64* expected, Score* score) {
  ...
  f64 err = 0;
  for (u64 i = 0; i < size; i++) { err += sqrt(pow(expected[i] - output_layer->neurons[i], 2)); }
  ...
  // expected 1 = positif
  if (expected[0] > 0.5) {
    if (err < 0.5) score->true_positive++;
    else
      score->false_negative++;
  } else {
    if (err < 0.5) score->true_negative++;
    else
